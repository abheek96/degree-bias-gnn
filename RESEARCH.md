...
**On the role of the degree advantage plot:** The degree advantage metric (`same_mean_deg − diff_mean_deg`) is an important piece of the puzzle — it captures whether the GCN normalisation factor further amplifies or partially compensates the count imbalance. A negative degree advantage (diff-class training neighbours have lower degree and therefore stronger per-edge weight) directly worsens the aggregation imbalance for a node that already has a count disadvantage. This makes the degree advantage plot essential for explaining certain anomalies that count-based metrics alone cannot account for. However, it does not tell the whole story either: a node can have a positive degree advantage yet still be misclassified if the influence signal is routed away from same-class neighbours by learned weights. Conversely, a node with a negative degree advantage can still be classified correctly if the count of same-class training neighbours is large enough. The full picture requires the degree advantage, the influence disparity, and the structural metrics (purity, SPL) to be read together — the anomalous cases are those where several of these factors simultaneously point in the wrong direction.
