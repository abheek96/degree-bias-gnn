...
# Suppress NumPy 2.x deprecation warning from torch_geometric's Planetoid loader
...
def main():
    ...
    if plot_cfg.get("max_same_train_deg", False):
        plot_max_same_train_deg_vs_degree(
            max_same_train_deg, test_deg, cfg,
            save_dir=exec_dir if plot_cfg.get("save", True) else None,
            show=plot_cfg.get("show", False),
        )
