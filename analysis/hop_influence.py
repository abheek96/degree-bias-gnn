"""
hop_influence.py — Per-hop Jacobian-L1 influence breakdown for a specific test node.
...
Usage
-----
    uv run analysis/hop_influence.py --node-idx 1362
    uv run analysis/hop_influence.py --node-idx 1362 2210 \\
...
def analyse_node_per_hop(model, data, pred, node_x: int, k_hops: int,
    ...
    def _edge_weight(n):
        return 1.0 / math.sqrt((int(all_deg[n].item()) + 1) * (degree + 1))

    def _node_tuples(nodes):
        if not nodes:
            return "—"
        return str(sorted(
            (int(all_deg[n].item()),
    ...
    table.align = "r"
    table.align[FIELDS[-2]] = "l"
    table.align[FIELDS[-1]] = "l"
